...
def fast_single_ring(X: np.ndarray, U: np.ndarray) -> float:
    """
...
def fast_msr_array(sync_data: np.ndarray, Tw: int, expected_size: int = 80, amplitude: float = 1.0) -> np.ndarray:
    """
    ...
    with tqdm(total=num_windows) as bar:
        ...
        for i in range(num_windows):
            ...
            msrs[i] = fast_single_ring(noise_data, U)
            bar.update()
    return msrs
